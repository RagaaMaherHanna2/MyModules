...
class APIAccessToken(models.Model):
    ...
    logout = fields.Boolean(defualt = False)


    def find_one_or_create_token(self, user_id=None , token=None, exp = None, create=False ):
        if not user_id:
            user_id = self.env.user.id

        access_token = self.env["api.access_token"].sudo().search([("user_id", "=", user_id) , ('token' ,'=' , token)], order="id DESC", limit=1)
        if access_token:
            access_token = access_token[0]
            if access_token.has_expired():
                access_token = None
        if not access_token and create:
            expires = datetime.now() + timedelta(seconds=24 * 60 * 60)
            #expires = exp
            vals = {
                "user_id": user_id,
                "scope": "userinfo",
