...
class AccountMove(models.Model):
    ...
    def _auto_create_asset(self):
        ...
        for move in self:
            ...
            for move_line in move.line_ids.filtered(lambda line: (move.move_type in ('out_invoice', 'in_invoice') and line.account_id.asset_type in ('sale', 'expense'))):
                if (
                    ...
                    if (move.move_type == 'out_invoice' and move_line.account_id.asset_type == 'sale') or (move.move_type == 'in_invoice' and move_line.account_id.asset_type == 'expense'):
...
class AccountMoveLine(models.Model):
    ...
    def turn_as_asset(self):
        return self._turn_as_asset('purchase', _("Turn as an asset"), self.env.ref("itq_deferred_expense_revenue.view_account_asset_form"))

    def turn_as_deferred(self):
        balance = sum(aml.debit - aml.credit for aml in self)
        if balance > 0:
            return self._turn_as_asset('expense', _("Turn as a deferred expense"), self.env.ref('itq_deferred_expense_revenue.view_account_asset_expense_form'))
        else:
            return self._turn_as_asset('sale', _("Turn as a deferred revenue"), self.env.ref('itq_deferred_expense_revenue.view_account_asset_revenue_form'))
