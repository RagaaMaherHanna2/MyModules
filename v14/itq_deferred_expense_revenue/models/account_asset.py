...
class AccountAsset(models.Model):
    ...
    @api.depends('account_depreciation_expense_id')
    def _compute_available_accounts(self):
        ...
        domain = [('user_type_id', 'in', [current_asset, current_liabilities]), ('company_id', '=', self.env.company.id), ('is_off_balance', '=', False)]
        accounts = self.env['account.account'].search(domain)
        for asset in self:
            if asset.asset_type == 'sale':
                filtered_accounts = accounts.filtered(lambda a: a.user_type_id.id == current_liabilities)
                asset.available_account_ids = [(6, 0, filtered_accounts.ids)]
            elif asset.asset_type == 'expense':
                filtered_accounts = accounts.filtered(lambda a: a.user_type_id.id == current_asset)
                asset.available_account_ids = [(6, 0, filtered_accounts.ids)]
            else:
                asset.available_account_ids = [(6, 0, accounts.ids)]
    ...
    def compute_depreciation_board(self):
        ...
        new_moves = self.env['account.move'].with_context(skip_partner_validation=True).create(newline_vals_list)
    ...
    def _get_views(self, type):
        form_view = self.env.ref('itq_deferred_expense_revenue.view_account_asset_form')
        tree_view = self.env.ref('itq_deferred_expense_revenue.view_account_asset_purchase_tree')
        if type == 'sale':
            form_view = self.env.ref('itq_deferred_expense_revenue.view_account_asset_revenue_form')
            tree_view = self.env.ref('itq_deferred_expense_revenue.view_account_asset_sale_tree')
        elif type == 'expense':
            form_view = self.env.ref('itq_deferred_expense_revenue.view_account_asset_expense_form')
            tree_view = self.env.ref('itq_deferred_expense_revenue.view_account_asset_expense_tree')
    ...
    def action_save_model(self):
        form_ref = {
            'purchase': 'itq_deferred_expense_revenue.view_account_asset_form',
            'sale': 'itq_deferred_expense_revenue.view_account_asset_revenue_form',
            'expense': 'itq_deferred_expense_revenue.view_account_asset_expense_form',
        }.get(self.asset_type)
